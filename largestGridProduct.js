...
function largestProductInSeries(arrLike, n) {
  let newProdInfo = newProd(arrLike, 0, n);
  
  let i = newProdInfo.i;
  let prod = newProdInfo.prod;
  let prodStart = newProdInfo.start;

  let maxProd = prod;
  // let maxProdStart = prodStart;

  while(arrLike.inBounds(i)) {
    if(prod === 0) {
      newProdInfo = newProd(arrLike, i, n);
      ...
      i = newProdInfo.i;
      prod = newProdInfo.prod;
      prodStart = newProdInfo.start;
    }
    else {
      prod = prod * arrLike.nth(i) / arrLike.nth(prodStart);
      prodStart++;
      i++;
    }
...
// Modified from largestProductinaSeries.js, but uses an "array-like" object instead of an array
function newProd(arrLike, start, len) {
  if(!arrLike.inBounds(start + len - 1)) {
    return undefined;
  }
